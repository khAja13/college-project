...
# ATTENDANCE VERDICT'S
class Attverdict(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    uid = db.Column(db.String(15), nullable=False)
    course = db.Column(db.String(30), nullable=False)
    section = db.Column(db.Integer, nullable=False)
...
def student_login_page():
    ...
    try:
        if session['stuid'] is not null:
            ...
            results = Teacherdetails.query.filter_by(section=r[0][2]).all()
            cur = conn.cursor()
            classes = []
            for values in results:
                tableto = values.courseid.lower() + str(values.section)
                query = "SELECT column_name FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = '{}'".format(tableto)
                cur.execute(query)
                result = cur.fetchall()
                classes.append(len(result) - 2)

            attendance = {}
            k=0
            for i, j in zip(forAtt, forSubjects):
                attendance[j] = [classes[k],i]
                k=k+1
            return render_template('/stu_profile.html', attendance = attendance, classes = classes)
    except:
...
def sec_att():
    ...
    cur = conn.cursor()
    tableto = r.courseid.lower() + str(r.section)
    query = "SELECT column_name FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = '{}'".format(tableto)
    cur.execute(query)
    result = cur.fetchall()
    classes = len(result) - 2
    
    return render_template('tea_sec_att.html', result=result, course=course, result1=result1, courseid=r.courseid.upper(), section=section, classes=classes)
...
def attendance():
    ...
    if result:
        ...
        query = Attverdict.query.filter_by(course=course, section=section).first()
        if query:
            started = query.started_at
            present = dt.datetime.now()
            endtime = started + timedelta(minutes = 15)
            
            if present > endtime:
                flash('Attendance time is over 😂')
                return render_template('/attendance.html', first=True)
...
def start_att():
    ...
    query = Attverdict.query.filter_by(course=course.upper(), section=section).first()
    if query:
    ...
    query = Attverdict(uid=teaid, course=course, section=section, started_at=dt.datetime.now())
    db.session.add(query)
